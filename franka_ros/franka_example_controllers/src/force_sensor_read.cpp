...
private:
    ...
    bool parseSensorData(const std::vector<uint8_t>& data, uint16_t& package_no, std::vector<double>& forces) {
        try {
            ...
            if (data.size() < 31 || data[0] != 0xAA || data[1] != 0x55) {
                ROS_DEBUG("数据头无效，数据大小: %zu", data.size());
                return false;
            }
    ...
    void readDataThread() {
        ...
        while (running_ && ros::ok()) {
            ...
            // 减小延迟以支持更高频率，但保持系统稳定
            std::this_thread::sleep_for(std::chrono::milliseconds(1)); // 1ms延迟，平衡性能和稳定性
        }
