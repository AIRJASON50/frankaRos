...
使用的机械臂：fr3（不带夹爪）
官方文档：
@frankaros/docs/Franka Control Interface Specifications.markdown;
@frankaros/docs/libfranka Documentation.markdown;
@frankaros/docs/Franka Control Interface Specifications.markdown
参考控制代码：
@frankaros/franka_example_controllers下的代码（circlecontroller相关内容是我写的，不作为参考）

力传感器的波特率：115200，参考：@M4313MXB Users' Manual V1.markdown 
获取读数的参考：@forcesenseRead.py（这是python脚本，我需要使用cpp进行获取）

所有的数值设置内容，比如阈值、轨迹尺寸等内容统一放在：
/home/jason/ws/catkin_ws/src/franka_ros/franka_example_controllers/config/simulation_parameters_config.yaml进行设置，其他文件要使用时在其中读取

我希望我的实验的控制器可以兼容实机测试和仿真，二者差别只是获取力传感的方式不同；

算法逻辑和要求：
    第一阶段：调零；
    在启动时，在初始状态下保持静止，（抵抗外力保持，我的系统中没有夹爪（gripper）默认的控制代码可能会有重力补偿，检查其他示例代码考察是否需要修改）
    思路都是窗口式获取均值计算offset值，并且将后来的力值都减去这个offset值使得力值趋向零；调零完成标志是力值稳定极小（5s内所有均值小于0.5n）当调零完成后，后续的z方向力都是使用调零后的力进行计算和判断；
